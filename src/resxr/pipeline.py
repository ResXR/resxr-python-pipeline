...
def write_dataset_files(config: PipelineConfig, bids: BIDSLayout, subjects: list[dict]) -> None:
    ...
    if not readme_path.exists() or config.output.overwrite:
        readme_content = config.bids.readme_text or f"{config.output.dataset_name}\n\nGenerated by ResXR pipeline.\n"
    ...
    if not deriv_readme_path.exists() or config.output.overwrite:
        deriv_readme_content = f"{config.output.dataset_name} (preprocessed)\n\nGenerated by ResXR pipeline.\n"
        deriv_readme_path.write_text(deriv_readme_content)
